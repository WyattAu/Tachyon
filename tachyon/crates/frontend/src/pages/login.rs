...
pub fn LoginPage() -> impl IntoView {
    ...
    let field_errors = RwSignal::new(Vec::<String>::new());
    ...
    let on_submit = move |ev: ev::SubmitEvent| {
        ...
        if username_val.trim().is_empty() {
            errs.push("Username or email is required".to_string());
        }
        if username_val.contains('@') && !is_valid_email(&username_val) {
            ...
            <div class="max-w-md w-full space-y-8">
                ...
                <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-8">
                    ...
                    <Show when=move || !field_errors.get().is_empty()>
                        <div class="mb-4 p-3 bg-yellow-50 dark:bg-yellow-900/30 border border-yellow-300 dark:border-yellow-700 text-yellow-700 dark:text-yellow-200 rounded text-sm">
                            <ul class="list-disc list-inside space-y-1">
                                {move || field_errors.get().into_iter().map(|e| view! { <li>{e}</li> }).collect::<Vec<_>>()}
                            </ul>
                        </div>
                    </Show>
