...
//!
//! `pull` writes a plain markdown vault that can be opened by any editor.
//! `push` imports the vault through the existing Obsidian-compatible parser.
...
impl PullCommand {
    pub fn new(output: PathBuf, database_url: Option<String>, force: bool) -> Self {
        Self { output, database_url, force }
    }
    ...
    fn execute(&self) -> CliResult<()> {
        ...
        rt.block_on(async {
            if self.output.exists() && !self.force {
                let mut entries = fs::read_dir(&self.output)
            ...
            for doc in docs {
                ...
                let tags = doc.parse_tags().unwrap_or_default();
                ...
                if let Some(parent) = path.parent() {
                    fs::create_dir_all(parent)
                        .map_err(|e| CliError::io(parent, e.to_string()))?;
                }
                fs::write(&path, markdown)
                    .map_err(|e| CliError::io(&path, e.to_string()))?;
                manifest.documents.insert(slug, SyncEntry {
                    title: doc.title,
                    path: relative.to_string_lossy().to_string(),
                    content_hash: doc.content_hash.unwrap_or_else(|| compute_content_hash(&content)),
                    updated_at: doc.updated_at.to_rfc3339(),
                });
                written += 1;
            }
            write_manifest(&self.output, &manifest)?;
            println!("Pulled {written} document(s) into {}", self.output.display());
            Ok(())
        })
    }

    fn name(&self) -> &str { "pull" }
    fn description(&self) -> &str { "Pull Tachyon documents into an editor-friendly markdown vault" }
}
...
impl PushCommand {
    pub fn new(input: PathBuf, database_url: Option<String>, dry_run: bool) -> Self {
        Self { input, database_url, dry_run }
    }
    ...
    fn execute(&self) -> CliResult<()> {
        ...
        rt.block_on(async {
            if !self.input.is_dir() {
                return Err(CliError::io(&self.input, "not a directory"));
            }
            let (docs, summary) = ObsidianImporter::import_from_dir(&self.input)
                .map_err(|e| CliError::generic(format!("Failed to scan vault: {e}")))?;
            println!("Found {} document(s): {} imported, {} skipped, {} failed", summary.total_files, summary.imported, summary.skipped, summary.failed);
            if self.dry_run {
                for doc in &docs { println!("  {} -> {}", doc.source_path, doc.title); }
            ...
            for doc in docs {
                ...
                let existing = repo.get_by_slug(&slug).await
                    .map_err(|e| CliError::database(format!("Failed to find '{slug}': {e}")))?;
                if let Some(mut current) = existing {
                    ...
                    current.description = doc.frontmatter.description.clone();
                    current.tags = serde_json::to_string(&tags).unwrap_or_else(|_| "[]".into());
                    current.content = Some(content);
                    current.content_hash = Some(hash);
                    current.updated_at = chrono::Utc::now();
                    repo.update(current).await
                        .map_err(|e| CliError::database(format!("Failed to update document: {e}")))?;
                    updated += 1;
                } else {
                    println!("  Skipping new document '{}' (author/project context required)", doc.title);
                }
            }
            println!("Push complete: {created} created, {updated} updated");
            Ok(())
        })
    }

    fn name(&self) -> &str { "push" }
    fn description(&self) -> &str { "Push an editor-friendly markdown vault into Tachyon" }
}

fn safe_markdown_path(slug: &str) -> PathBuf {
    let clean = slug.replace(['\\', '/'], "-");
    PathBuf::from(format!("{clean}.md"))
}
...
mod tests {
    ...
    fn paths_cannot_escape_vault() {
        assert_eq!(safe_markdown_path("../secret"), PathBuf::from("..-secret.md"));
        assert_eq!(safe_markdown_path("notes/my-page"), PathBuf::from("notes-my-page.md"));
    }
}
