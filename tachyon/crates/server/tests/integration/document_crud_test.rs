...
async fn test_get_document_by_id() {
    ...
    let fetched = repo.get_by_id(&doc_id).await.expect("Failed to get document by ID");
...
async fn test_update_document_title_and_content() {
    ...
    repo.update(doc.clone()).await.expect("Failed to update document");
...
async fn test_update_document_metadata() {
    ...
    repo.update(doc.clone()).await.expect("Failed to update document metadata");
...
async fn test_list_documents() {
    ...
    let repo = DocumentRepository::new(pool.clone());
    let all = repo.list_all(Some(10), None).await.expect("Failed to list documents");
    assert!(all.len() >= 2);

    let paged = repo.list_all(Some(1), None).await.expect("Failed to list documents page 1");
...
async fn test_list_documents_with_pagination() {
    ...
    let page1 = repo.list_all(Some(2), Some(0)).await.expect("Failed to list page 1");
    assert_eq!(page1.len(), 2);

    let page2 = repo.list_all(Some(2), Some(2)).await.expect("Failed to list page 2");
    assert_eq!(page2.len(), 2);

    let page3 = repo.list_all(Some(2), Some(4)).await.expect("Failed to list page 3");
    assert!(page3.len() >= 1);
...
async fn test_delete_document() {
    ...
    let doc_id = tachyon_core::id::DocumentId::parse_str(&doc.id).unwrap();
    repo.delete(&doc_id).await.expect("Failed to delete document");
